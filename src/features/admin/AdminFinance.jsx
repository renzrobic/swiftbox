...
export default function AdminFinance() {
  return (
    <section className="space-y-8 text-left w-full max-w-full overflow-hidden" aria-labelledby="finance-heading">
      <div>
        <h2 id="finance-heading" className="text-2xl font-semibold text-ink">
          Financials
        </h2>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {FINANCIAL_METRICS.map((item) => (
          <div 
            key={item.id} 
            className="bg-white border border-ink/10 p-8 rounded-xl lg:rounded-2xl shadow-sm relative overflow-hidden group"
          >
            <p className="text-[10px] font-semibold text-ink/80 tracking-wider mb-2">
              {item.label}
            </p>
            <div className="flex items-end gap-3">
              <h3 className="text-3xl font-semibold text-ink tracking-tighter leading-none">
                {item.val}
              </h3>
              <span 
                className="text-[10px] font-semibold text-green-500 mb-1 flex items-center"
      ...
      <div className="bg-ink rounded-xl lg:rounded-2xl p-8 md:p-12 text-white shadow-xl">
        <h4 className="text-[10px] font-semibold tracking-wider opacity-80 mb-10 text-center">
          Revenue Projection
        </h4>
        ...
        >
          {PROJECTION_DATA.map((h, i) => (
            <div key={`bar-${i}`} className="flex-1 flex flex-col items-center gap-4 h-full justify-end">
              ...
              >
                {/* Tooltip */}
                <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-white text-ink px-2 py-1 rounded-lg text-[10px] font-semibold opacity-0 group-hover:opacity-100 transition-all shadow-xl pointer-events-none">
                  ₱{h * 100}
                </div>
              </div>
              <span className="text-[9px] font-semibold opacity-80 tracking-wider">Day {i + 1}</span>
            </div>
          ))}
        </div>
      </div>
