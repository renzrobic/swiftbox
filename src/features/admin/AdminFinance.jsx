...
export default function AdminFinance() {
  return (
    <section className="space-y-8 text-left w-full max-w-full overflow-hidden" aria-labelledby="finance-heading">
      <div>
        <h2 id="finance-heading" className="text-2xl font-bold text-royal-blue uppercase">
          Financials
        </h2>
      </div>
        ...
        {FINANCIAL_METRICS.map((item) => (
          ...
          >
            ...
            <div className="flex items-end gap-3">
              <h3 className="text-3xl font-bold text-royal-blue tracking-tighter leading-none">
                {item.val}
              </h3>
              <span 
      ...
      <div className="bg-royal-blue rounded-xl lg:rounded-2xl p-8 md:p-12 text-white shadow-xl">
        ...
        >
          {PROJECTION_DATA.map((h, i) => (
            <div key={`bar-${i}`} className="flex-1 flex flex-col items-center gap-4 h-full justify-end">
              ...
              >
                {/* Tooltip */}
                <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-white text-royal-blue px-2 py-1 rounded-lg text-[10px] font-black opacity-0 group-hover:opacity-100 transition-all shadow-xl pointer-events-none">
                  ₱{h * 100}
                </div>
              </div>
