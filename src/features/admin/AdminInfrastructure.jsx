...
const StatItem = memo(({ icon: Icon, value, label }) => {
  return (
    <div className="flex items-center gap-1.5" aria-label={`${label}: ${value}`}>
      ...
      <span className="text-[9px] font-semibold text-ink/80">{value}</span>
    </div>
  );
});
...
export default function AdminInfrastructure() {
  return (
    <section className="space-y-6 md:space-y-10 text-left w-full overflow-hidden">
      <div>
        <h2 className="text-2xl font-semibold text-ink">Infrastructure</h2>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8">
            ...
            {NODES.map((node) => (
              <div key={node.name} className="bg-white border border-ink/10 p-6 rounded-xl lg:rounded-2xl shadow-sm hover:border-ink/20 transition-all">
                <div className="flex justify-between items-center mb-8">
                  <span 
                    className={`text-[8px] font-semibold tracking-wider px-2 py-1 rounded-md ${
                ...
                <h4 className="text-lg font-semibold text-ink leading-none tracking-tight">
                  {node.name}
                </h4>
                <p className="text-[9px] font-semibold text-ink/80 mt-2">
                  {node.location}
                </p>
              </div>
            ))}
        ...
        <aside className="lg:col-span-4 bg-white border border-ink/10 rounded-xl lg:rounded-2xl p-6 shadow-sm h-fit">
          <div className="flex items-center gap-2 mb-8">
            ...
            <h3 className="text-[10px] font-semibold tracking-wider text-ink/80">Queue</h3>
          </div>
          
          <div className="space-y-6">
            {MAINTENANCE_LOGS.map((log) => (
              <div key={`${log.target}-${log.task}`} className="border-l-2 border-ink/5 pl-4 py-0.5">
                <p className="text-xs font-semibold text-ink leading-tight">{log.task}</p>
                <p className="text-[9px] font-semibold text-ink/80 mt-1">
                  {log.target} • {log.time}
                </p>
              </div>
            ))}
      ...
      >
        ...
        <div>
          <p className="text-[10px] font-semibold text-amber-700 tracking-wider leading-none">
            Power Critical: Node_PAG_03
          </p>
          <p className="text-[9px] font-semibold text-amber-600/80 mt-1">
