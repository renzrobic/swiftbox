...
const ArticleRow = memo(({ art, onEdit, onDelete }) => (
  <div className="group flex items-center justify-between rounded-xl border border-ink/10 bg-white p-4 transition-all hover:shadow-sm lg:rounded-2xl">
    <div className="flex items-center gap-4">
      ...
      <div>
        <h4 className="line-clamp-1 text-sm font-semibold text-ink">{art.title}</h4>
        <p className="text-[9px] font-semibold text-ink/80">
        ...
        <form onSubmit={handleSave} className="space-y-6 rounded-2xl bg-white p-8 shadow-sm border border-ink/10">
          <div className="flex flex-col items-start gap-4">
            ...
            <button 
              ...
              className="rounded-xl bg-ink/5 px-6 py-3 text-[10px] font-semibold text-ink transition-all hover:bg-ink hover:text-white"
            >
          ...
          <input 
            ...
            className="w-full rounded-xl bg-ink/5 p-4 text-xl font-semibold text-ink outline-none placeholder:text-ink/30 border border-transparent focus:border-ink/20" 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <select 
              ...
              onChange={handleChange} 
              className="cursor-pointer rounded-xl bg-ink/5 p-4 text-xs font-semibold text-ink outline-none border border-transparent focus:border-ink/20"
            >
          ...
          <button 
            type="submit" 
            disabled={loading} 
            className="mt-4 flex w-auto items-center justify-center gap-2 rounded-xl bg-ink px-10 py-4 font-semibold tracking-wider text-white shadow-lg transition-all hover:bg-black disabled:opacity-50"
          >
  ...
  return (
    <div className="space-y-8 text-left">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-semibold text-ink">Newsroom</h2>
        <button 
          type="button"
          onClick={() => { setCurrentArticle(INITIAL_ARTICLE_STATE); setIsEditorOpen(true); }} 
          className="rounded-xl bg-ink px-6 py-3 text-[10px] font-semibold tracking-wider text-white shadow-md transition-all hover:bg-black active:scale-95 lg:rounded-2xl"
        >
          New Entry
        </button>
      </div>
