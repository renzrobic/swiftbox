...
const TEAM = [
  {
    id: 1, name: "Renz Robic L. Bernal", surname: "Bernal", role: "Project Lead & Lead Developer", png: "Bernal.png",
  ...
  {
    id: 2, name: "Jarwin D. Villar", surname: "Villar", role: "Hardware & Research Support", png: "Villar.png",
  ...
  {
    id: 3, name: "Kim Philip C. Ramos", surname: "Ramos", role: "Research & Survey Lead", png: "Ramos.png",
  ...
  {
    id: 4, name: "Arman Grey C. Carpio", surname: "Carpio", role: "Hardware & Research Support", png: "Carpio.png",
  ...
  {
    id: 5, name: "Japher G. Padayhag", surname: "Padayhag", role: "Hardware & Research Support", png: "Padayhag.png",
...
const Card = memo(({ member, isActive, isCollapsed, onToggle }) => {
  return (
    <div
      ...
      className={`relative flex flex-col md:flex-row overflow-hidden border-2 cursor-pointer transition-[flex,border-color,box-shadow,background-color] duration-[650ms] ease-[cubic-bezier(0.34,1.26,0.64,1)] rounded-[1.5rem] ${
        isActive
          ? 'flex-[12] border-transparent bg-white shadow-[0_0_30px_rgba(13,13,13,0.1)] active-glow'
          : 'flex-1 border-ink/5 bg-white hover:border-ink/20 hover:shadow-md'
      }`}
    >
      {/* ── Photo Pane ── */}
      <div
      ...
      >
        ...
        <div
          className={`absolute bottom-4 left-0 z-20 hidden w-full text-center font-semibold tracking-wider text-white transition-opacity duration-350 md:block ${
      ...
      >
        ...
        >
          <div className="mb-4 flex items-start justify-between">
            <span className="font-mono text-[10px] font-semibold tracking-wider text-ink/20">
              {member.idDisplay}
            </span>
            <div className="text-right">
              <h3 className="text-xl font-semibold leading-none tracking-tight text-ink md:text-4xl">
                {member.name}
              </h3>
              <p className="mt-1.5 text-[8px] font-semibold tracking-wider text-ink/80 md:text-[11px]">
                {member.role}
              </p>
            </div>
          </div>
          ...
          <div className="mb-6 shrink-0">
            <p className="text-[9px] font-semibold tracking-wider text-ink/80 md:text-[10px]">
              Academic Profile
            </p>
            <p className="text-xs font-semibold tracking-tight text-ink/90 md:text-lg">
              {member.courseId}
            </p>
          </div>
          ...
          <div className="flex shrink-0 flex-wrap gap-2 border-t border-ink/5 pt-5">
            {member.tags.map((tag) => (
              <span
                key={tag}
                className="rounded-full border border-ink/10 bg-ink/[0.05] px-3 py-1.5 text-[8px] font-semibold tracking-tighter text-ink/80 md:text-[10px]"
              >
                {tag}
              </span>
            ))}
...
export default function TeamAccordion() {
  ...
  return (
    <section id="team" className="section-container section-spacing select-none font-sans">
