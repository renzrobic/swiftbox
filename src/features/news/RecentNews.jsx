      ...
      <div className="mx-auto max-w-7xl px-8 md:px-10">
        ...
        {loading ? (
          <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-80 animate-pulse rounded-2xl bg-ink/5" />
            ))}
          </div>
        ) : articles.length > 0 ? (
          /* Success State */
              ...
              >
                <Link to={`/news/${art.id}`} className="group block text-left">
                  ...
                  <div className="mt-8">
                    ...
                    <h3 className="text-xl font-bold leading-tight tracking-tight text-ink transition-colors group-hover:text-matte-charcoal">
                      {art.title}
                    </h3>

                    <div className="mt-6 flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-ink opacity-0 transition-all group-hover:translate-x-2 group-hover:opacity-100">
