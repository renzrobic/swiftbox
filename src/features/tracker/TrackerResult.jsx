      ...
      <div className="relative flex-1 h-[400px] lg:h-auto border-b lg:border-b-0 lg:border-r border-ink/5 overflow-hidden bg-pearl">
          ...
          {mapLocked && (
            ...
            >
              ...
              >
                ...
                <span className="text-xs font-semibold">Unlock Live Map</span>
              </button>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="absolute bottom-6 left-6 z-[1000] hidden md:block">
          <div className="bg-white/90 backdrop-blur-xl p-4 px-6 rounded-2xl border border-ink/5 shadow-xl">
            <p className="text-[10px] font-medium text-ink/30 mb-1">Live Node Positioning</p>
            <h4 className="text-xl font-semibold text-ink tracking-tight">{parcelData.location}</h4>
          </div>
        </div>
      </div>
      ...
      <aside className="w-full lg:w-[350px] flex flex-col bg-white overflow-hidden">
        ...
        <div className="p-6 border-b border-ink/5">
          <div className="flex items-center justify-between mb-6">
            ...
            <span className={`text-[10px] px-2.5 py-0.5 rounded-full font-medium ${isAtSchool ? 'bg-ink text-white' : 'bg-ink/5 text-ink/30'}`}>
              {isAtSchool ? 'Armed' : 'Active'}
            </span>
          </div>
          
          <p className="text-[10px] font-medium text-ink/50 mb-1.5">Locker Authorization</p>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
          <div className="flex items-center justify-between">
            <h5 className="text-[10px] font-medium text-ink/50">Event Stream</h5>
            ...
            {history.map((step, i) => (
              <div key={i} className="flex gap-5 relative group/step">
                ...
                <div className="flex flex-col">
                  <p className={`text-xs font-semibold tracking-tight ${step.current ? 'text-ink' : 'text-ink/40'}`}>
                    {step.status}
                  </p>
                  <p className={`text-[10px] font-medium mt-1 ${step.current ? 'text-ink/60' : 'text-ink/20'}`}>
                    {step.location}
                  </p>
                </div>
              </div>
        ...
        <div className="p-6 bg-pearl border-t border-ink/5 mt-auto">
          <div className="flex justify-between items-center">
            <span className="text-[10px] font-medium text-ink/50">Origin Source</span>
            <span className="text-[11px] font-semibold text-ink">{parcelData.sender}</span>
          </div>
        </div>
      </aside>
