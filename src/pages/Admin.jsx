...
import { AdminSkeleton } from '../components/skeletons/PageSkeleton';
    ...
    <div className="flex h-screen w-full flex-col overflow-hidden bg-white pt-24 text-left font-sans text-ink md:flex-row md:pt-24">
      ...
      <aside className="hidden w-64 flex-col justify-between border-r border-ink/5 bg-white p-6 md:flex">
        <div>
          <header className="mb-10 px-2">
            <h1 className="text-2xl font-semibold leading-none tracking-tight text-ink">
              {userSession?.name}
            </h1>
            <p className="mt-1 text-[10px] font-semibold tracking-wider text-ink/80">
              {userSession?.role}
            </p>
          </header>
      ...
      <nav className="flex flex-col border-b border-ink/10 bg-white md:hidden overflow-hidden">
        <div className="flex items-center justify-between p-4 px-6">
          <span className="text-[11px] font-semibold tracking-wider text-ink/80">Swiftbox Terminal</span>
          <button
            onClick={logout}
          ...
          >
            ...
            {currentTab === 'settings' && (
              <div className="flex min-h-[50vh] flex-col items-center justify-center">
                ...
                <div className="text-4xl font-semibold text-ink/80">Config v2.06</div>
                <p className="mt-2 text-[10px] font-semibold tracking-wider text-ink/80">Core System Parameters</p>
              </div>
            )}
          </motion.div>
        </AnimatePresence>
